...
export async function getAllDailyTransactions(req: Request, res: Response, next: NextFunction) {
  try {
    console.log(req.query);
    const { page, limit, transaction_date, search_id_cashier_name, cashier_shift } = req.query;
    ...
    if (search_id_cashier_name) {
      whereClause.OR = [
        ...
        },
        {
          cashier: {
            OR: [
              { firstName: { contains: String(search_id_cashier_name), mode: 'insensitive' } },
              { lastName: { contains: String(search_id_cashier_name), mode: 'insensitive' } },
            ],
          },
        },
      ];
    ...
    let transactions = await prisma.transaction.findMany({
        include: {
          cashier: { select: { id: true, firstName: true, lastName: true, shift: true } },
          cashierBalanceHistory: { select: { id: true, startingCash: true, endingCash: true } },
          transactionItems: {
            include: {
              product: {
                select: { id: true, name: true, price: true, productCategory: { select: { id: true, name: true } } },
              },
            },
          },
        },
...
export async function getAllMonthlyTransactions(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const groupedByDate = dailyTransactions.reduce(
      ...
      {} as Record<string, { date: string; totalAmount: number; transactionCount: number }>
    );
...
export async function getAllDailyItemTransaction(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Apply pagination
    const paginatedItems = itemsWithDetails.slice((pageNumber - 1) * pageLimit, pageNumber * pageLimit);
...
export async function getAllCashierTransactions(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const shiftGroups = transactions.reduce(
      (acc, transaction) => {
        ...
        if (transaction.cashierBalanceHistory) {
          acc[shift].startingCash += transaction.cashierBalanceHistory.startingCash;
          acc[shift].endingCash += transaction.cashierBalanceHistory.endingCash || 0;
        }
    ...
    // Apply pagination
    const paginatedItems = shiftTransactions.slice((pageNumber - 1) * pageLimit, pageNumber * pageLimit);
